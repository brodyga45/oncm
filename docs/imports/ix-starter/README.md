...
## Shared importer contract (not installed in apps yet)

This implements the **materials** side of US-012/US-013 in [user scenarios](../../user-scenarios.md). An application can copy this compact catalogue into its own independent runtime bundle, verify the manifest, show three cards and fill a draft from `oncmSourcePath`. Show repository/commit/declaration, custom TN definition and verification statuses before the user creates a market. Importing does not resolve anything and does not establish description/goal equivalence by name alone.

All three independent apps can consume the same format without cross-app runtime imports. A future generic remote adapter must pin dependencies and select a complete audited declaration closure; it must not scrape arbitrary theorem lines or execute Ix's entire test harness. For this starter, the reviewed selection is explicit and bounded.
...
A new upstream commit intentionally requires a reviewed new snapshot and excerpt/dependency pins. There is no automatic assertion that a changed theorem retains its old statement ID or certificate. No app, chain, installed profile, runtime policy, or proving asset was changed for this catalogue.
