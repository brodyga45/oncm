...
Status: the first remote run `34414002347` executed the complete perf05 checker (17.28 ms guest execution), then its 4 GiB host scope received SIGKILL after 78.28 seconds during proving, before Docker started. No receipt was created. This is consistent with the host cgroup memory limit; that first recipe did not save kernel OOM counters. The correction described below shares the existing 13 GiB total budget across both phases and captures those counters. **Corrected run [34415338599](https://github.com/brodyga45/oncm/actions/runs/34415338599) is now in progress** with `profile=perf05`, `case=true-registration`; setup and seven lightweight checks passed. Its real proof outcome is pending. The only success marker is `verified.json`, written after the original `r0vm` accepts the real Groth16 receipt for the pinned image and the exact 128-byte journal matches.
...
- **Shared parent:** one new systemd slice per case, `MemoryMax=13G`, `MemorySwapMax=0`, `CPUQuota=200%`, `TasksMax=256`. Before any proof work, setup requires cgroup v2, Docker's `systemd` driver and the system daemon; it does not reconfigure or restart Docker. Runtime reads back the actual parent `memory.max`, `memory.swap.max`, `cpu.max` and required diagnostic counters. A mismatch stops the case. The same flat slice is passed to host `systemd-run --slice=…` and Docker `--cgroup-parent=…`; Docker's daemon-launched process therefore shares the parent's limit. [Official Docker systemd slice naming and per-container cgroup parent](https://docs.docker.com/reference/cli/dockerd/#default-cgroup-parent).
- Host and descendants have a 12 GiB child limit; original prover Docker keeps its 9 GiB child limit, two-CPU cap, no network and `GOMAXPROCS=2` / `GOMEMLIMIT=8GiB`. **12 + 9 are not independent budgets:** the kernel enforces the shared 13 GiB parent across both children, including overlap. Aggregate CPU is also capped at two CPUs by the parent. This lets each phase use otherwise idle allowance while retaining the previous whole-workload ceiling; it does not assume that host buffers have already been freed when Docker starts. These CI limits **do not meet the local 2 GiB target**. [systemd v255 resource-control source](https://github.com/systemd/systemd/blob/v255/man/systemd.resource-control.xml), [kernel cgroup v2 accounting/limits](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html).
