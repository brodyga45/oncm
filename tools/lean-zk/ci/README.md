...
Status: the first remote run `34414002347` executed the complete perf05 checker (17.28 ms guest execution), then its 4 GiB host scope received SIGKILL after 78.28 seconds during proving, before Docker started. No receipt was created. This is consistent with the host cgroup memory limit; that first recipe did not save kernel OOM counters. The correction described below shares the existing 13 GiB total budget across both phases and captures those counters. **The corrected recipe has not yet been run remotely.** First retry should still use `profile=perf05`, `case=true-registration`. The only success marker is `verified.json`, written after the original `r0vm` accepts the real Groth16 receipt for the pinned image and the exact 128-byte journal matches.
