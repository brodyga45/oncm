...
export async function deployStack(client=publicClient,wallet=devWallet(),accounts=devAccounts.map(a=>a.address),{mock=false,seed=true}={}){
 await assertLocalChain(client);const deployed={};
 ...
 const manifest={app:'Agora',chainId:31371,rpc:'http://127.0.0.1:9545',deployedAt:new Date().toISOString(),token,ctf,allocation,factory,safe,timelock,registry,verifier,profileId,profileManifest,operatorCandidate,proofStatus,council:accounts.slice(0,2),accounts,deployed,startBlock:'0',mock};
 if(seed&&mock){
