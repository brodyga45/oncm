...
import {encodeAbiParameters,decodeAbiParameters,sha256,stringToBytes as toUtf8Bytes} from 'viem';
...
const directory=new URL('../proof/additional-profiles/perf05/',import.meta.url);
const read=name=>fs.readFileSync(new URL(name==='true-registration.json'?'registration.json':name,directory));
const profile={...JSON.parse(read('profile.json')),goals:[],fixtures:{},verifierParameters:'0x73c457ba541936f0d907daf0c7253a39a9c5c427c225ba7709e44702d3c6eedc',selector:'0x73c457ba'};
const options={trustedProfile:profile,foundationBytes:read('fixtures/foundation.ndjson')};
const abi={encode:(types,values)=>encodeAbiParameters(types.map(type=>({type})),values),decode:(types,value)=>decodeAbiParameters(types.map(type=>({type})),value)};
function wrap(name='true-registration'){
 ...
 return {format:'oncm-external-certificate-bundle-v1',artifact:JSON.parse(read(name+'.json')),
...
test('base64/size/SHA, foundation bytes and exact goal boundary are checked',()=>{
 ...
 const trailing=wrap();rebindGoal(trailing,Buffer.concat([read('fixtures/true-goal.ndjson'),Buffer.from('{}\n')]));
...
test('supported v3 descriptor validates its own goal bytes without pretending perf05 seal proves v3',()=>{
 const readV3=p=>fs.readFileSync(new URL('../proof/'+p,import.meta.url));const v3={...JSON.parse(readV3('manifest.json')),verifierParameters:profile.verifierParameters,selector:profile.selector,fixtures:{}};
 const b=wrap(),goal=readV3('lean/goal.ndjson');b.artifact.profileId=v3.profileId;b.artifact.imageId=v3.imageId;rebindGoal(b,goal);
 const fields=abi.decode(['bytes32','bytes32','bytes32','uint256'],b.artifact.journal);fields[2]=v3.profileId;b.artifact.journal=abi.encode(['bytes32','bytes32','bytes32','uint256'],fields);b.artifact.certificate=abi.encode(['bytes','bytes'],[b.artifact.evmSeal,b.artifact.journal]);
 const result=inspectExternalBundle(b,{trustedProfile:v3,foundationBytes:readV3('lean/foundation.ndjson')});assert.equal(result.profileId,v3.profileId);assert.equal(result.cryptographicStatus,'not-verified');assert.equal(result.sourceGoalRelation,'not-verified');
});
