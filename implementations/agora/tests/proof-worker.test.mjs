...
import { createProofWorker } from '../server/proof-jobs.mjs';

async function setup(t) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'agora-live-worker-test-'));
...
test('live worker adapter uses bounded supervisor for a tiny dummy and writes its report', async t => {
  ...
  const report = JSON.parse(await fs.readFile(path.join(root, '.local/proof-resource-reports/small.json')));
...
test('live worker adapter settles cancellation only after supervisor cleanup', async t => {
  ...
  const report = JSON.parse(await fs.readFile(path.join(root, '.local/proof-resource-reports/cancel.json')));
