...
export async function transact(wallet,address,abi,functionName,args=[],value=0n){await assertLocalChain();if(await wallet.getChainId()!==31371)throw new Error('Switch wallet to Agora chain 31371');const {request}=await publicClient.simulateContract({account:wallet.account,address,abi,functionName,args,value});const hash=await wallet.writeContract(request);const receipt=await publicClient.waitForTransactionReceipt({hash});if(receipt.status!=='success')throw new Error('Transaction reverted');return receipt;}
