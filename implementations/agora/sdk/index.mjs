import {createPublicClient,http,parseEther,zeroHash,decodeEventLog} from 'viem';
...
export async function createAgoraSDK({wallet,config,client,apiUrl='http://127.0.0.1:4171/api'}={}){
 ...
 const approveT=(spender,amount)=>write(config.token,'TrueToken','approve',[spender,amount]);
 const quote=async(pool,side,amount,mode='buy')=>{if(side!==0&&side!==1)throw new Error('Outcome is 0=YES or 1=NO');return read(pool,'FixedProductMarketMaker',mode==='buy'?'calcBuyAmount':'calcSellAmount',[amount,side]);};
 return{
  ...
  createPool:(id,fee=parseEther('.02'))=>write(config.registry,'AgoraRegistry','createPool',[id,fee]),
  ...
  removeLiquidity:(pool,shares)=>write(pool,'FixedProductMarketMaker','removeFunding',[shares]),
  ...
  merge:(conditionId,amount)=>write(config.ctf,'ConditionalTokens','mergePositions',[config.token,zeroHash,conditionId,[1n,2n],amount]),
