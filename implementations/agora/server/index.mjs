...
import {composeGovernanceCall,reviewGovernanceAction} from './governance-actions.mjs';
import {readTreasury} from '../sdk/treasury.mjs';
...
// Encapsulated certificate routes inherit the handler at registration time.
...
await app.register(cors,{origin:network.webOrigin,credentials:true});app.setReplySerializer(stringify);
...
app.get('/api/treasury',async()=>{const block=await pc.getBlock();return readTreasury({read,config:manifest,blockNumber:block.number});});
const currentGovernance=()=>governanceSnapshot({client:pc,read,manifest,proposals:db.governance,rpcUrl:chain.rpcUrls.default.http[0],reviewAction:(proposal,block)=>reviewGovernanceAction({...proposal,read,manifest,abis,block})});
...
app.post('/api/governance',async req=>{
 const author=session(req).address,b=req.body,call=composeGovernanceCall(b,manifest,abis),{target,data}=call,block=await pc.getBlock();
 const actionContext=await reviewGovernanceAction({...call,read,manifest,abis,block});
 const salt=keccak256(toHex(crypto.randomUUID())),delay=await read(manifest.timelock,'AgoraTimelock','getMinDelay');
 const scheduleData=encodeFunctionData({abi:abis.AgoraTimelock,functionName:'schedule',args:[target,0n,data,zeroHash,salt,delay]});
 const nonce=await read(manifest.safe,'Safe','nonce'),safeArgs=[manifest.timelock,0n,scheduleData,0,0n,0n,0n,zeroAddress,zeroAddress,nonce];
 const hash=await read(manifest.safe,'Safe','getTransactionHash',safeArgs),operationId=await read(manifest.timelock,'AgoraTimelock','hashOperation',[target,0n,data,zeroHash,salt]);
 const p={id:crypto.randomUUID(),title:String(b.title??'Governance decision'),author,target,data,salt,operationId,scheduleData,nonce:nonce.toString(),safeHash:hash,signatures:[],createdAt:new Date().toISOString(),...(call.action?{action:{...call.action,...(actionContext?.baseVersion!==undefined?{baseVersion:actionContext.baseVersion}:{})},actionContext}:{})};
 db.governance.push(p);save();return p;
});
app.post('/api/governance/:id/signatures',async req=>{const p=db.governance.find(x=>x.id===req.params.id);if(!p)throw new Error('Unknown proposal');const current=(await currentGovernance()).proposals.find(x=>x.id===p.id);if(!current?.canSign)throw Error(current?.actionError??'Proposal is no longer signable');const address=await recoverMessageAddress({message:{raw:p.safeHash},signature:req.body.signature});if(!manifest.council.some(a=>a.toLowerCase()===address.toLowerCase()))throw new Error('Council signature required');p.signatures=p.signatures.filter(s=>s.address.toLowerCase()!==address.toLowerCase());p.signatures.push({address,signature:req.body.signature});save();return p;});
