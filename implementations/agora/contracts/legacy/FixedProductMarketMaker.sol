...
interface IAllocationEpoch { function currentSplit() external view returns(address); function currentEpoch() external view returns(uint); }
...
contract FixedProductMarketMaker is ERC20, ERC1155TokenReceiver {
    ...
    function buy(uint investmentAmount, uint outcomeIndex, uint minOutcomeTokensToBuy) public {
        _requireTrading();
        ...
        feePoolWeight = feePoolWeight.add(feeAmount.sub(feeAmount / 5));
    ...
    function sell(uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell) public {
        _requireTrading();
