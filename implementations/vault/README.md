...
В **Lean Lab → Импорт сертификата и отдельный профиль** можно загрузить настоящий `true-registration` из публичного CI или собственный `verified.json` до64KiB. Кнопка проверки выполняет только `eth_call`: точный profile/image, все128bytes journal, outcome, параметры SHA2Groth16, seal256/260bytes и canonical `abi.encode(seal,journal)` проверяются перед вызовом **оригинального** RISC Zero verifier. Поле `evmVerified` из JSON не принимается на веру. Затем отдельно проверяется immutable bridge соответствующего профиля и его фактический допуск в registry.
...
Настоящий CI4 `true-proof` уже включён в standalone bundle: **Загрузить true-proof из CI** → **Проверить original EVM и профиль** → **Применить к выбранному утверждению** → отдельная отправка proof ончейн. Для NO используется канонический case `false-refutation` с outcome2; `false-proof` отклоняется. Настоящий `false-registration` также включён в bundle; сертификат refutation принимается через тот же импорт, когда он получен. Перед применением нужно выбрать существующее утверждение с точно совпадающими goal/profile. После EVM проверки импорт заполнит outcome1 и certificate; кнопка отправки proof вызовет обычный `StatementRegistry.submitProof`. Новый proof нельзя применить к v3-рынку. Локальный runner остаётся v3; его кнопки для отдельного perf05 выключены, никакой новый prover не стартует при импорте. Допущенный ранее, но выключенный для новых регистраций профиль сохраняет возможность разрешения своих существующих рынков.
