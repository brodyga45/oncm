...
Активный V2 social graph отдельно проверен в [браузерном проходе 553–563](evidence/monetary-policy/v2-social-browser/README.md): профиль, комментарии/ответы, смена/снятие голосов, редакции, блог со скрытием/восстановлением и публичное восстановление из EAS после перезагрузки.
...
## Активная версия и запуск

Основная разработка ведётся в **Vault V2**: T выпускается только настоящим Timelock по решению Governor; бюджеты наград отделены от дохода выгодополучателей. [Положительный ручной цикл до блока 552](docs/US023-BROWSER-RESULT.md) включает реальный Lean/Groth16-рынок, торговлю двух кошельков, комиссии, награды, LP-выход и погашение.

Для уже развёрнутой V2, включая текущий сохранённый стенд:
...
Команда выбирает `.state/deployment-v2.json` и отдельный социальный реестр; она не превращает старый токен в новый и не разворачивает V2 при отсутствии descriptor. [Выбор версий и точные проверки](docs/protocol-versions.md). При первом создании самостоятельного стенда подготовьте базовую сеть по инструкции ниже, затем выполните явный просмотр плана `node scripts/deploy-v2.mjs` и отдельный deployment `node scripts/deploy-v2.mjs --execute` по [описанию развёртывания V2](MONETARY-POLICY-V2.md). Допуск профилей и первичная эмиссия затем проходят обычное голосование в сайте. Не запускайте deployment повторно поверх существующей V2.

### Базовая сеть и сохранённый Legacy

Без `VAULT_PROTOCOL_VERSION=2` запуск по совместимости выбирает Legacy. Следующая инструкция описывает именно эту исходную версию и её базовые зависимости; genesis-эмиссия ниже относится к Legacy.
...
Вход: кнопка кошелька → Browser wallet либо явно обозначенный публичный local devnet account 0–3. Последние доступны только для loopback RPC согласованного port offset и chain 31373. Anvil выдаёт тестовые ETH. В Legacy каждый из четырёх аккаунтов получает фиксированно 1,000,000 T при развёртывании, последующего mint нет. В активной V2 genesis supply равен нулю; первичный и последующие выпуски выполняет governance, автоматически выданных T у новых пользователей нет. Токены и обеспечение двух версий различаются.
