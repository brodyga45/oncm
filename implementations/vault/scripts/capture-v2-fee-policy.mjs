...
try{
 ...
 const tx=blocks.flatMap(b=>b.transactions);assert.deepEqual(tx.map(x=>x.decoded.method),['propose','castVote','castVote','queue','execute']);
 ...
 const report={format:'vault-v2-browser-governed-fee-policy-v1',chainId:31373,chainInstance:config.chainInstance.id,pool,beforeBlock:spec.beforeBlock,finalBlock:spec.afterBlock,blocks,proposal:{id:String(proposalId),description:spec.description,target:target.target,method:spec.method,calldata:call,snapshot:String(snapshot),deadline:String(await governor.proposalDeadline(proposalId,at(spec.afterBlock))),quorum:String(quorum),votes:clean(votes),state:'Executed'},fees:{before:beforeFees,after:afterFees},protectedState:{before,after,unchanged:true},scope:'Actual ordinary browser Governor receipts; historical eth_call state only. No capture transaction, trade, issuance, reward, resolution or impersonation.'};
