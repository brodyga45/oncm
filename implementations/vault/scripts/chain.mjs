...
try {
  ...
  throw Error(`RPC 9547 already responds (HTTP ${response.status}); reuse it with npm run dev.`);
} catch (error) {
...
const args = ['--host', '127.0.0.1', '--port', '9547', '--chain-id', '31373', '--hardfork', 'cancun',
...
console.log(`Vault Anvil RPC :9547 · instance ${instance.id} · state every 10 s`);
