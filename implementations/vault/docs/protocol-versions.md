...
Legacy descriptor/ABI/social descriptor bytes, its9 statements, epoch4, original T supply and tested wallet/CTF collateral balances match before and after. New initial allocation is Bob40% / Timelock15% / Alice45%, a copy of current proportions without old funds or claims.28 deployed/reused runtime hashes were checked. The governed v3/perf05 profile calls and operator admission are prepared but not submitted by the deployment; the existing verifier reference is retained only to permit independent cryptographic checking. Social setup/runtime switching and ordinary governance issuance are separate stages, with their later evidence recorded above. [17 receipts, events and invariant comparison](../evidence/monetary-policy/deployment-261-277.json), [exact pending calls](../evidence/monetary-policy/pending-governance-calls.json), [version implementation and recovery](../MONETARY-POLICY-V2.md).
