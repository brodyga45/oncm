...
const id='0x9b78961455b75385a452ae8278e749b2a897d75eb615bde01f43726a64e49d7f';
...
const blocks=[];
for(let number=158;number<=through;number++){
 ...
 for(const transaction of b.transactions){
  ...
  const before=number===158?null:await balances(number-1),after=await balances(number),delta={};if(before)for(const a of accounts){delta[a]={};for(const symbol of Object.keys(assets))delta[a][symbol]=String(BigInt(after[a][symbol])-BigInt(before[a][symbol]));}
