...
export function DerivedResolution({m,sdk,address,run,busy}) {
  ...
  async function refresh(){
    ...
    try{const next=await sdk.derivedReadiness(m);if(guard.current.current(ticket))setSnapshot(next);}
  ...
  return <div className="panel">
    ...
    <p>The contract evaluates the recorded dependency outcome and timestamp. Passing a deadline does not send a transaction automatically.</p>
    <dl><dt>Statement</dt><dd><code>{m.id}</code></dd><dt>Dependency</dt><dd><code>{m.dependency}</code></dd>
      <dt>Predicate</dt><dd>{['Lean','ResolvedBy','ResolvedAs','ResolvedAsBy','Governance operator'][m.kind]}</dd>
      <dt>Required outcome</dt><dd>{m.kind===1?'Either outcome':m.kind===4?'Determined by operator':m.targetOutcome===1?'True':'False'}</dd>
