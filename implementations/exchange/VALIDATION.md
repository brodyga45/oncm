...
## Activity identity and public snapshot import completion

The browser's duplicate balance rows were caused by different address casing. Normalized identities now deduplicate actors/assets before historical RPC reads while preserving first display labels and separate assets. Actual block242 recheck showed18 unique rows instead of24, with all exact before/after/delta values unchanged. The API-only restart preserved all65 prior JSON/policy fingerprints, all six terminal jobs and head242; proving stayed disabled. [Activity evidence](docs/evidence/activity-identity/README.md) separates this actual browser observation from the targeted tests and one bounded2.59s production build.

The actual **Palomar library → Import exact snapshot & goal** path imported public entry `PALOMAR-2026-09-08-000004-v1`, repository `wstrinz/configuration-23-4` at `94fc8964562fa9e246c8c7b1657e2e135bda35f6`, into an unverified draft. `Config23.exists_configuration`, source and metadata were restored; all seven explicit files matched their saved SHA256 and the1548-byte source matched the DOM digest. Goal/certificate stayed empty and Create disabled. The form was cancelled without a transaction or Lean job. The profile default is not a compatibility attestation for the imported Lean4.32.0/Mathlib project. [Public import evidence](docs/evidence/palomar-browser/README.md) records the exact limits: no full dependency closure, rebuild, canonical export or certificate generation. The final lightweight identity plus portable-package regression run passed9/9 in266.909ms; stale-certificate clearing is additionally covered by source review and tests, without claiming an in-place browser race reproduction.

## DAO treasury preparation and polling correction

The DAO helper reuses the actual Governor/Timelock, AllocationController and original SplitsWarehouse, with no new contract or executor wallet. Same-block reads bind the DAO address; reduction consent/revocation, governed Warehouse claims and exact LP/T ERC20 transfers use ordinary Governor proposals. The20% shortcut prepares only a reviewed allocation draft. [DAO-TREASURY.md](docs/DAO-TREASURY.md) records addresses, ABI, scope and review.

An actual read-only browser check caught an old Fee income interval closure overwriting prepared text after7s. The correction preserves edited/empty drafts and invalidates changed epoch/executor reviews without overwriting user input. Twenty targeted tests passed, including the actual component's captured callback and exact reviewed SDK arguments. Final bounded Vite passed2.43s, supervisor2.693s/603,392,976B. At265, actual prepared text/review survived23.117s; manual edits survived19.823s and matched the regenerated review and offline original ABI. DAO credit was zero, so claim preparation refused before any proposal. [Browser evidence](docs/evidence/dao-treasury/browser-poll-265.json) makes no positive payment or consent claim. The subsequent authorized live income/consent phase is separate; an internal payment proposal requires the coordinator's explicit release.


## Actual positive cycle and final Exchange freeze

The browser completed DAO admission, genuine V2 protocol LP income, a governed DAO-only Warehouse claim and governed consent to reduce the DAO share, through block319. Epoch4 is DAO15/Bob40/Alice45. DAO holds918830339724520 raw units (0.00091883033972452) of YES/T LP; its Warehouse withdrawable credit is zero. Both Governor proposals are Executed. [Full evidence](docs/evidence/dao-treasury/through-319.json) records20 successful transactions and historical conservation assertions. No CTF collateral or T supply changed.
